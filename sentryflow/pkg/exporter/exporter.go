...
func Init(ctx context.Context, server *grpc.Server, cfg *config.Base, events chan *protobuf.APIEvent, wg *sync.WaitGroup) error {
	logger := util.LoggerFromCtx(ctx).Named("exporter")
	logger.Info("Starting exporter")

	if cfg == nil {
		return fmt.Errorf("no exporter config found")
	}

	grpcCfg := cfg.Grpc
	if grpcCfg == nil {
		return fmt.Errorf("no grpc config found")
	}
