...
const (
	...
	SentryFlow_GetAPILogV2_FullMethodName     = "/protobuf.SentryFlow/GetAPILogV2"
...
type SentryFlowClient interface {
	...
	GetAPILogV2(ctx context.Context, in *ClientInfo, opts ...grpc.CallOption) (grpc.ServerStreamingClient[APILogV2], error)
...
func (c *sentryFlowClient) GetAPILogV2(ctx context.Context, in *ClientInfo, opts ...grpc.CallOption) (grpc.ServerStreamingClient[APILogV2], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SentryFlow_ServiceDesc.Streams[1], SentryFlow_GetAPILogV2_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ClientInfo, APILogV2]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SentryFlow_GetAPILogV2Client = grpc.ServerStreamingClient[APILogV2]
...
type SentryFlowServer interface {
	...
	GetAPILogV2(*ClientInfo, grpc.ServerStreamingServer[APILogV2]) error
...
func (UnimplementedSentryFlowServer) GetAPILogV2(*ClientInfo, grpc.ServerStreamingServer[APILogV2]) error {
	return status.Errorf(codes.Unimplemented, "method GetAPILogV2 not implemented")
}
...
func _SentryFlow_GetAPILogV2_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ClientInfo)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(SentryFlowServer).GetAPILogV2(m, &grpc.GenericServerStream[ClientInfo, APILogV2]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SentryFlow_GetAPILogV2Server = grpc.ServerStreamingServer[APILogV2]
...
var SentryFlow_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "GetAPILogV2",
			Handler:       _SentryFlow_GetAPILogV2_Handler,
			ServerStreams: true,
		},
		{
